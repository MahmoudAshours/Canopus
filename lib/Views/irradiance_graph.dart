...
class IrradianceGraphScreen extends StatefulWidget {
  final Map<String, dynamic>? value;
  final String? name;

  const IrradianceGraphScreen({Key? key, required this.value, this.name})
      : super(key: key);

  @override
  State<IrradianceGraphScreen> createState() => _IrradianceGraphScreenState();
}

class _IrradianceGraphScreenState extends State<IrradianceGraphScreen> {
  int _currentIndex = 0;
...
class GraphTypes {
  ...
  ChartSeries<String, String> fastLine() {
    return FastLineSeries<String, String>(
      ...
      dataSource:
          value.keys.where((element) => value[element] != -999.0).toList(),
      xValueMapper: (sales, _) => sales,
